#!/usr/bin/env python3
"""
Example script for TTS synthesis using CosyVoice3 model.
...
def tts_cosyvoice3(
    ...
    """
    Synthesize speech using CosyVoice3 model.
    ...
    print(f"Synthesizing with CosyVoice3 ({mode} mode)...")
...
def demo_all_modes(voice_id: str, text: str = "Hola, esto es una prueba de síntesis de voz con CosyVoice3."):
    """Demonstrate all TTS modes with CosyVoice3."""
    print("=" * 60)
    print("COSYVOICE3 TTS DEMO - ALL MODES")
    print("=" * 60)
    ...
    for i, config in enumerate(modes):
        ...
        output_file = f"cosyvoice3_{config['mode']}_{'stream' if config['stream'] else 'download'}.wav"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TTS synthesis with CosyVoice3")
    parser.add_argument("--text", default="Hola, esto es una prueba de síntesis de voz con CosyVoice3.", help="Text to synthesize")
